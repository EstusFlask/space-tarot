...
export default function Header({
  ...
  const brandTitleWidth = currentScreen === 'spread_selection' ? 'max-w-[34vw]' : 'max-w-[6rem]';
  const supportActionsClass = currentScreen === 'spread_selection' ? 'flex' : 'hidden sm:flex';

  return (
    <header className="liquid-glass-bar fixed top-0 w-full z-50 border-b border-white/10 flex justify-between items-center px-3 md:px-6 h-16 transition-all duration-300">
      <div className="flex min-w-0 items-center gap-2 md:gap-3">
        <button
          ...
          className="liquid-glass-control flex h-9 shrink-0 items-center justify-center gap-2 rounded-full px-3 text-[var(--theme-accent)] transition-all duration-200 hover:text-[var(--theme-fg-strong)] hover:drop-shadow-[0_0_8px_rgba(165,231,255,0.8)] active:scale-95 cursor-pointer"
        >
          <Sparkles className="h-4 w-4 shrink-0 drop-shadow-[0_0_8px_rgba(165,231,255,0.8)] md:h-5 md:w-5" />
          <span className={`${brandTitleWidth} truncate whitespace-nowrap font-serif text-base font-bold tracking-widest md:max-w-none md:text-xl`}>
            {copy.title}
          </span>
        </button>
        ...
        <div className={`${supportActionsClass} shrink-0 items-center gap-2`}>
          <button
